...
class VSMMeasurement(Measurement):
    def __init__(
        ...
        if metadata:
            ...
            self.const_temp = metadata.get("const_temp") or metadata.get(
                "const_temperature"
            )
    ...
    def process_data(self, raw_df, filepath) -> pd.DataFrame:
        ...
        if self.mode == "MT":
            self.const_field = (
                np.average(df.filter(regex="Magnetic Field")).round().astype(int)
            )
            for col in df.columns:
                if re.match(r"Moment", col):
    ...
    def __repr__(self):
        if self.mode == "MT":
            return (
                f"{self.mode} on {self.sample_name} "
    ...
    def plot(
        ...
        **kwargs,
        ...
        if self.mode == "MT":
            ...
            print(
                'Ah oh, something went wrong. Check if measurement.mode is "MH" or "MT".'
            )

        # ax.set_ylabel('Susceptibility (emu)')
        ax.set_title(
            f"{self.sample.name} {self.mode} {self.sample.orientation if self.sample.orientation else ''}"
        )
        ax.legend()
    ...
    def fit_MH(self, fit_window=[0, 70000], window_length=101, p0=None):
        ...
        dMdH_detrended_filtered = savgol_filter(
            dMdH_detrended, window_length, polyorder=3
        )
        ...
        ax[0].scatter(
            ExtField, dMdH_detrended_filtered, s=1, label="Detrended Filtered Data"
        )
        ax[0].plot(
            ExtField, gauss(ExtField, *popt), "-", lw=2, label="Guassian fit", color="g"
        )
        ax[0].axvline(
            x0_fit, color="r", ls="--", label=f"Peak @ {x0_fit:.0f} Oe, FWHM = {FWHM}"
        )
        ax[0].legend()
    ...
    def fit_MT(
        ...
        # --- 1. 等间距插值 & 平滑 χ(T) -----------------------------------
        f_interp = interp1d(
            Temp, chi, kind="linear", bounds_error=False, fill_value="extrapolate"
        ...
        chi_smooth = savgol_filter(
            chi_resampled, window_length=smooth_window, polyorder=3
        )
        ...
        deriv_smooth_window = min(
            deriv_smooth_window, len(dchidT_trend_removed) // 2 * 2 + 1
        )
        dchidT_detrended_filtered = savgol_filter(
            dchidT_trend_removed, deriv_smooth_window, 3
        )
        ...
        ax[0].plot(
            T_new_clean, dchidT_detrended_filtered, ".", ms=3, alpha=0.5, label="data"
        )
        ax[0].plot(x_fit, y_fit, ".", ms=5, label="fit region")
        ax[0].plot(
            T_new_clean, fit_func(T_new_clean, *popt), "-", lw=2, label="Guassian fit"
        )
        ax[0].axvline(
            x0_fit,
            color="r",
            ls="--",
            label=f"Peak @ {x0_fit:.2f} T, FWHM = {FWHM:.3f}",
        )
        ...
        ax[1].scatter(
            T_new_clean, dchidT_detrended_filtered, label="Detrended+Filtered", s=1
        )
